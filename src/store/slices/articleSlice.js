   ...
   extraReducers: (builder) => {
            ...
            if (article.slug === action.payload.article.slug) {
               ...
               if (!liked)
                  localStorage.setItem(
                     'liked',
                     JSON.stringify([{ slug: action.payload.article.slug, profile: action.payload.profile }]),
                  );
               else
                  localStorage.setItem(
                     'liked',
                     JSON.stringify([...liked, { slug: action.payload.article.slug, profile: action.payload.profile }]),
                  );
      ...
      builder.addCase(unFavoriteIcon.fulfilled, (state, action) => {
         state.getArticle.articles.map((article) => {
            if (article.slug === action.payload.article.slug) {
               const liked = JSON.parse(localStorage.getItem('liked'));
               const resLiked = liked.filter((item) => {
                  if (item.slug === action.payload.article.slug) return false;
                  else return true;
               });
