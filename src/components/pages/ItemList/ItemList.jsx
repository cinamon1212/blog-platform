...
export function ItemList(props) {
   ...
   const { createdAt, slug, title, favoritesCount, tagList, description, author, body, date } = Object.keys(props)
      .length
   ...
   const onIconClick = () => {
      ...
      let isSlugIncluded = false;

      liked &&
         liked.forEach((element) => {
            if (element.slug === slug) isSlugIncluded = true;
         });

      if ((!liked || !isSlugIncluded) && token) favoriteIcon({ slug: slug, token, profile: newPerson.user.username });
      else if (token) unFavoriteIcon({ slug: slug, token, profile: newPerson.user.username });
   };
   ...
   liked &&
      liked.forEach((element) => {
         if (element.profile === newPerson?.user?.username && element.slug === slug) isSlugIncluded = true;
      });
         ...
         <div className={classes['list__item-description']}>
            <header className={classes['list__item-header']}>
               ...
               <div className={classes['list__item-icon']}>
                  <div
                     className={
                        !Object.keys(newPerson).length || !isSlugIncluded
